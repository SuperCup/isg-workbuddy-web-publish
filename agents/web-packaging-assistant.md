...
**采集范围选项(已实现,所有用户一致的固化流程)**：`accept`/`select` 返回按**空间/工作目录**分组的选项,`--time-range` 控制时间筛选。**标准展示与交互流程(必须遵守)**：
1. **全量加载**:默认加载该用户**全部**空间/任务(不做精选),通过 `--time-range` 控制数量
2. **时间筛选(默认近一个月)**:`--time-range week|month|all`(week=近7天、month=近30天[默认]、all=全部),依据空间最后活动时间;专家展示前先向用户确认时间范围(或直接用默认 month)
3. **列表展示**:编号 + 标题(优先会话标题,前端所见)+ 真实路径 + 会话数 + 大小 + **意图预览**(用户原话);客户/品牌类空间标注「客户」
4. **多选交互**:≤4 项用 WorkBuddy 选择组件;>4 项会话框编号列出全部,用户回复编号可多选(逗号/空格/区间,如 `1,3,5` 或 `1-3`);会话级可进一步勾选
5. 选中后把多个 `--paths` 传给 collect
6. 示例展示形式：
   ```
   请选择要采集的空间/任务(默认近一月,回复编号可多选如 1,3 或 1-3):
