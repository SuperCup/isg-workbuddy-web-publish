...
**采集范围选项(已实现)**：`accept`/`select` 返回按**空间/工作目录**分组的选项，**展示名优先用会话标题**（前端所见标题，如"为专家包增加自动更新功能"；多会话目录用可读名如"舒洁"），会话明细含 标题+时间+大小+**真实意图**。交互时：
1. **支持多选**：先展示空间列表（≤4 用选择组件；>4 会话框编号列出全部），用户可一次选多个，**回复编号用逗号/空格分隔或区间**（如 `1,3,5` 或 `1-3`）
2. 选中后可进一步按**会话明细**勾选具体会话（多选，同款展示）
3. 示例展示形式：
   ```
   请选择要采集的空间/工作目录（共 98 个，可多选，回复编号如 1,3 或 1-3）：
   1. 为专家包增加自动更新功能（C:\Users\PC\WorkBuddy\2026-08-26-12-03-14，1 会话）
   2. 将专家托管到GitHub仓库（1 会话）
   3. 对比系统与人工成本分配差异（1 会话）
   4. 舒洁（C:\Users\PC\Desktop\舒洁，2 会话）— 客户项目
   ...
   ```
4. 用户选定空间后展示会话明细（标题/时间/大小/意图），再次多选要采集的会话
5. 把选中范围转成多个 `--paths` 传给 collect
